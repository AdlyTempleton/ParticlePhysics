...
public class ContainerExtractor extends Container {
	...
	public ContainerExtractor(InventoryPlayer par1InventoryPlayer,
			ExtractorMachineTileEntity tileEntity) {
		...
		for (var3 = 3; var3 < 9; ++var3) {
			this.addSlotToContainer(new Slot(tileEntity, var3 - 3,
					8 + var3 * 18, 10));
		}

		for (var3 = 0; var3 < 9; ++var3) {
			this.addSlotToContainer(new Slot(tileEntity, var3 + 6,
					8 + var3 * 18, 36));
		}

		for (var3 = 0; var3 < 9; ++var3) {
			this.addSlotToContainer(new Slot(tileEntity, var3 + 15,
					8 + var3 * 18, 55));
		}
	...
	public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int par2) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (par2 < 24) {
				if (!this.mergeItemStack(itemstack1, 24,
