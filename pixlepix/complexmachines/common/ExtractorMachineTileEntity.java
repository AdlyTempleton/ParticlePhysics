...
public class ExtractorMachineTileEntity extends TileEntityElectricityRunnable implements IPacketReceiver, IElectricityStorage, IInventory{
    ...
    public final int[] ORE_LIST={14,15,16,21,56,73,129,458,688,3002,3880,3970,3989}; 
                ...
                else
                {
                	...
                	while(!oreFound&&getJoules()>1000){
	                	...
	                	*/
	                	...
	                	if(worldObj.getChunkFromBlockCoords(targetX,targetZ).isChunkLoaded&&matchedSlot!=-1){
	                		oreFound=true;
    	...
    	if(chestFound != null){
    			...
    			for(int i=0;i<chestFound.getSizeInventory();i++){
    				if(chestFound.isStackValidForSlot(i,drop)){
    					if(chestFound.getStackInSlot(i)!=null){
	    					int itemsToAdd=drop.stackSize;
	    					int currentItems=chestFound.getStackInSlot(i).stackSize;
	    					int newTotal=itemsToAdd+currentItems;
	    					chestFound.setInventorySlotContents(i, new ItemStack(chestFound.getStackInSlot(i).getItem(),newTotal));
    					}else{
    ...
    private TileEntityChest findChest(){
    	...
    	TileEntity entityBeingChecked=worldObj.getBlockTileEntity(xCoord,yCoord,zCoord);
    	...
    	if(worldObj.getBlockTileEntity(xCoord,yCoord+1,zCoord) instanceof TileEntityChest){
    		return (TileEntityChest)worldObj.getBlockTileEntity(xCoord,yCoord+1,zCoord);
    	}
    	if(worldObj.getBlockTileEntity(xCoord,yCoord-1,zCoord) instanceof TileEntityChest){
    		return (TileEntityChest)worldObj.getBlockTileEntity(xCoord,yCoord-1,zCoord);
    	}
    	if(worldObj.getBlockTileEntity(xCoord,yCoord,zCoord+1) instanceof TileEntityChest){
    		return (TileEntityChest)worldObj.getBlockTileEntity(xCoord,yCoord,zCoord+1);
    	}
    	if(worldObj.getBlockTileEntity(xCoord,yCoord,zCoord-1) instanceof TileEntityChest){
    		return (TileEntityChest)worldObj.getBlockTileEntity(xCoord,yCoord,zCoord-1);
    	}
